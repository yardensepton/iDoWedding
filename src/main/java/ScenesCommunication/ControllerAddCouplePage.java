...
public class ControllerAddCouplePage {
    ...
    public void checkValidation(MouseEvent mouseEvent) throws Exception {
        ...
        if (date == null) {
            ...
            alert.showAndWait();
        } else if (firstNameSpouse1.isEmpty() || lastNameSpouse1.isEmpty() || firstNameSpouse2.isEmpty() || lastNameSpouse2.isEmpty() || idSpouse1.isEmpty() || idSpouse2.isEmpty()) {
            Alert alert = new Alert(Alert.AlertType.ERROR, "Empty fields..");
            alert.showAndWait();
        } else {
            try {
                Main.manager.addCouple(firstNameSpouse1, lastNameSpouse1, idSpouse1, firstNameSpouse2, lastNameSpouse2, idSpouse2, guests, date, hall);
                Alert alert = new Alert(Alert.AlertType.CONFIRMATION, "couple added successfully");
                alert.showAndWait();
